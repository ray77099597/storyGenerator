...
public class ConceptToFabulaElementFilter_0109 {
	...
	public Set <String> filter(String cpt) {
		...
		tags = tagger.tagConcept(cpt);
		cptSplit = cpt.split("_");
		tagList = new LinkedList<String>(Arrays.asList(tags));
		cptList = new LinkedList<String>(Arrays.asList(cptSplit));
	...
	private boolean isAction() {
		if(cptList.size() == 1){
			if(tagger.isVerb(tagList.get(0)) && wkt.isVerb(cptList.get(0))){
